<script lang="ts">
	...
	const delegateSimpleIsLeft = async (
		gp: string | null,
		gender: string | null,
		isDesc: boolean | true,
		...
		const response =
			errorToNull(
				await justPostStatistics<DelegateIsLeft[]>('is_left_per_delegate', {
					legis_period: gp,
					party: null,
					gender,
					is_desc: isDesc,
	...
	const delegateSimpleIsLiberal = async (
		gp: string | null,
		gender: string | null,
		isDesc: boolean | true,
		normalized: boolean | true
		...
		const response =
			errorToNull(
				await justPostStatistics<DelegateIsLiberal[]>('is_left_per_party', {
					legis_period: gp,
					party: null,
					gender,
					is_desc: isDesc,
	...
	const partySimplePoliticalPosition = async (
		gp: string | null,
		gender: string | null,
		isDesc: boolean | true,
		normalized: boolean | true
		...
		const response =
			errorToNull(
				await justPostStatistics<PoliticalPosition[]>('is_left_per_party', {
					legis_period: gp,
					party: null,
					gender,
					is_desc: isDesc,
...
<Container>
	<DelegateBarChartControl
		...
		id={0}
		delegateMakeRequest={delegateSimpleIsLeft}
		title="Links Rechts Wert pro Abgeordneten"
	/>
</Container>
...
<Container>
	<DelegateBarChartControl
		height={tableHeightDelegate}
		id={1}
		delegateMakeRequest={delegateSimpleIsLiberal}
		title="Liberal Autoritär Wert pro Abgeordneten"
	/>
</Container>
