<script lang="ts">
    ...
    if ($modalStore.length > 0) messages = [
        { role: 'assistant', content: `Ich bin SomBOTka, ein Chatbot vom Demokratieprojekt "somes". Ich bin spezialisiert darauf, Fragen aus der Sicht von ${$modalStore[0].meta.delegate.name} (${$modalStore[0].meta.delegate.party}) auf verschiedene Themen zu beantworten.` },
    ];
    ...
    const sendMessage = () => {
        ...
        if ($modalStore.length > 0 && newMessage.length > 0) {
            ...
            messages = [...messages, { role: 'user', content: sentMessage }, { role: 'assistant', content: "" }];
            chatSocket.send(JSON.stringify(
                {"question": sentMessage, "delegate_id": $modalStore[0].meta.delegate.id, "chat_history": chatHistory }
            ));
    ...
    <div class="flex-1 p-4 overflow-y-auto bg-gray-50 dark:bg-gray-900">
        {#each messages as { role, content }}
            <div class={`flex mb-4 ${role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div class={`max-w-[70%] px-4 py-2 rounded-lg text-sm ${role === 'user' ? 'bg-secondary-500 text-white' : 'bg-primary-400 text-gray-900 dark:text-gray-100'}`}>{content}</div>
            </div>
        {/each}
    </div>
    <div class="flex items-center border-t border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800">
        <input type="text" bind:value={newMessage} placeholder="Stelle deine Frage..." on:keypress={(e) => e.key === 'Enter' && sendMessage()} class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-full text-base outline-none focus:ring focus:ring-primary text-black" />
        <button on:click={sendMessage} class="ml-4 px-4 py-2 bg-primary-500 text-white rounded-full hover:bg-primary-800 focus:outline-none focus:ring focus:ring-primary-dark">Senden</button>
    </div>
</div>