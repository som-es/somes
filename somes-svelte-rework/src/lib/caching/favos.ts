...
export async function cachedDelegateFavos(refetch: boolean = false): Promise<Set<number> | null> {
...
export async function cachedLegisInitFavos(refetch: boolean = false): Promise<Set<number> | null> {
	let maybeCached = favoDelegates;
	if (maybeCached == null || refetch || maybeCached.size == 0) {
