...
<Container>
	{#if step === 'start'}
		...
		<div class="mt-6 grid gap-4 sm:grid-cols-2">
			<button class="btn card border-black border p-6 text-left" onclick={() => startQuiz('short')}>
				<h3 class="h3">Kurz</h3>
				<p class="mt-2 text-sm">Weniger Fragen, schneller Überblick.</p>
			</button>
			<button class="btn card border-black border p-6 text-left" onclick={() => startQuiz('long')}>
				<h3 class="h3">Lang</h3>
						...
						{#if strongRefMode}
							{#if q.strong_reference_answers.length >= 2}
								...
								<div class="mt-2 flex justify-between gap-4">
									<span class="max-w-[33%]">{pro.answer}</span>
									<span class="max-w-[33%] text-right">{contra.answer}</span>
								</div>
							{:else}
						...
						{:else}
							<div class="mt-4 grid grid-cols-5 gap-2">
								{#each ['Stark dagegen', 'Dagegen', 'Neutral', 'Dafür', 'Stark dafür'] as label, i}
									<button
										class="btn bg-primary-400 {answers[q.id] === i * 25 ? 'bg-secondary-500 text-white' : ''}"
