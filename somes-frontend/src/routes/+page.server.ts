...
export const load: PageServerLoad = async ({ fetch, setHeaders }) => { 
    ...
    let platformItems: PlatformItem[] = [];
    ...
    if (!isHasError(latestVotes)) {
        latestVotes.forEach(vote => {
            platformItems.push({
                ...
                status: vote.legislative_initiative.accepted == null ? 'pending' : (vote.legislative_initiative.accepted == "a" ? 'accepted' : 'rejected')
            });
        });
    }
    ...
    if (!isHasError(latestMinisterialProposals)) {
        latestMinisterialProposals.forEach(proposal => {
            platformItems.push({
                id: randomId(),
                type: 'proposal',
                title: proposal.gov_proposal.ai_summary?.short_title ?? proposal.gov_proposal.ministrial_proposal.title,
