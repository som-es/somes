    ...
    if (!isHasError(voteResult)) {
        ...
        if (voteResult.referenced_by_others_ids.length > 0) {
            const results = await Promise.all(
                voteResult.referenced_by_others_ids.map(id => vote_result_by_id(id.toString(), fetch))
            );
        ...
        if (voteResult.references && voteResult.references.length > 0) {
            const results = await Promise.all(
                voteResult.references.map(ref => vote_result_by_path(ref.gp, ref.ityp, ref.inr.toString(), fetch))
            );
