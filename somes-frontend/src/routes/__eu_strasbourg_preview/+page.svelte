<script lang="ts">
	...
	onMount(async () => {
		...
		const del = delegates.find((delegate) => {
			if (delegate.name.includes('AUBRY')) return delegate;
		});
		// console.log(delegates);
		console.log(del);
		const seatMapping = await (
			await fetch(`http://localhost:5173/seat_to_circle.json`, {
				method: 'GET',
				headers: {
	...
	let circles2d: Bubble[][] = $derived.by(() => {
		...
		seatsCount.forEach((seat, idx) => {
			innerCircles2d.push(
				generateHalfCircle(seat, 70 + idx * 10, width, height, 260).map((circle) => {
					id += 1;
					return {
						r,
						x: circle.x,
		...
		delegates.forEach((delegate) => {
			if (delegate.seat_row && seats.length > 0) {
				...
				if (delegate.seat_row == 533) {
					...
					innerCircles2d[seat.circle - 1][seat.slot_in_circle_with_gaps - 1].color =
						'rgb(255, 0, 0)';
				}
				innerCircles2d[seat.circle - 1][seat.slot_in_circle_with_gaps - 1].opacity = 1;
				innerCircles2d[seat.circle - 1][seat.slot_in_circle_with_gaps - 1].color = partyToColor(
					delegate.party
				);
			}
		});
