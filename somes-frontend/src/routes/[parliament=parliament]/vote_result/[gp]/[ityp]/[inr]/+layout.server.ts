...
// loads all heavy data and caches it, independet from url state
export const load: LayoutServerLoad = async ({ fetch, params, setHeaders }) => {
	...
	const voteResult = await vote_result_by_path(
		params.gp,
		params.ityp,
		params.inr,
		fetch,
	...
	if (!isHasError(voteResult)) {
		...
		if (voteResult.referenced_by_others_ids.length > 0) {
			const results = await Promise.all(
				voteResult.referenced_by_others_ids.map((id) =>
					vote_result_by_id(id.toString(), fetch, parliament)
				)
			);
		...
		if (voteResult.references && voteResult.references.length > 0) {
			const results = await Promise.all(
				voteResult.references.map((ref) =>
					vote_result_by_path(ref.gp, ref.ityp, ref.inr.toString(), fetch, parliament)
				)
			);
