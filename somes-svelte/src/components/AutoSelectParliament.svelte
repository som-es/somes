<script lang="ts">
	...
	const currentDelegateStorage: Writable<Bubble | null> = localStorageStore(
		"currentDelegate",
		null,
	);
			...
			<div class="card w-28 border" style="border-color: {activeSelection.color};">
				<header>
					{#if currentImg}
						<img
							src={currentImg.src}
							class="bg-black/50 w-full aspect-[10/9] rounded"
							alt="image of politician {activeSelection.del.name}"
						/>
					{/if}
				...
				<section class="p-0.5 dark:text-white" style="font-size: 9px;">
					{activeSelection.del.name}
				</section>
			</div>
		{/if}
	</div>
