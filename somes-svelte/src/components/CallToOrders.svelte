<script lang="ts">
		...
		function update(data: DelegateByCallToOrders[]) {
			...
			u.enter()
				...
				.attr("width", function (d) {
					return x(d.call_to_order_amount);
				})
			...
			texts
				...
				.attr("x", function (d) {
					return x(d.call_to_order_amount) - 120;
				})
				.attr("width", "130")
				.attr("height", "50")
	...
	async function updateWithLegisData(period: string) {
		...
		if (period == "all") {
			...
			delegatesByCallToOrders = (await delegates_by_call_to_orders_and_legis_period(period)).slice(
				0,
				10,
			);
		}
