<script lang="ts">
	...
	onMount(async () => {
		...
		sliceDelegatesByCallToOrders.forEach((val) => {
			data0.push({ group: val.name, value: val.hours_spoken });
		});
		...
		function update(data: SpeakerByHours[]) {
			console.log(data);
			...
			var texts = svg.selectAll(".data-label").data(data);
	...
	async function updateWithLegisData(period: string) {
		...
		if (period == "all") {
			...
			delegatesByCallToOrders = (await speakers_by_hours_and_legis_period(period)).slice(0, 10);
		}
