		...
		<input
			...
			on:click={async () => {
				...
				if (res.insufficient_password) {
					pwdErrorMessage =
						"Passwort muss bestimmte Kriterien erfüllen! (definiere, grafisch darstellen..)";
				}
			}}
		/>
	</div>
{:else}
	<div class="login_container flex flex-col">
		...
		<h4 class="text-center mt-2">
			Verifizierungsmail an <span class="font-bold text-green-400">{email}</span> versendet.
		</h4>
		<p class="mt-1 text-center">
			Bitte überprüfe deine E-Mail und klicke auf den Link, um deine Registrierung abzuschließen
		</p>
	</div>
