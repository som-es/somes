...
pub struct GenderIsLiberalFilter {
...
pub async fn is_liberal_per_gender(
    ...
    Json(filter): Json<Option<GenderIsLiberalFilter>>,
) -> Result<Json<Vec<GenderIsLiberal>>, StatisticsResponse> {
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, GenderIsLiberal>(&query);
