...
pub struct DelegateIsLeftFilter {
...
pub async fn is_left_per_delegate(
    ...
    Json(filter): Json<Option<DelegateIsLeftFilter>>,
) -> Result<Json<Vec<DelegateIsLeft>>, StatisticsResponse> {
    ...
    let filter_arg3 = Manual("(m.is_nr OR m.is_gov_official)").into_filter_arg();
    ...
    SELECT DISTINCT ON (ds.id) 
        ...
        (ROUND((pop.is_left - pop.is_not_left)::numeric, 5)::FLOAT) + 1 AS left_score
    FROM 
    ...
    GROUP BY
        ds.id, ds.name, ds.party, pop.is_left, pop.is_not_left
    ORDER BY 
        ds.id DESC  
) subquery
ORDER BY left_score {desc};
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, DelegateIsLeft>(&query);
