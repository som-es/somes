...
pub async fn activity_per_delegate(
    ...
    SELECT 
        ...
        )::float8 AS normalized_absences_per_session
    FROM 
        proposals p
    JOIN 
    ...
    GROUP BY 
        d.id,
        d.name,
        mandate_duration.mandate_duration_days
    ORDER BY 
        normalized_absences_per_session DESC;
    "
    );
    ...
    filtered_query
        ...
        .map(Json)
        .map_err(|e|{
            println!("Database query failed (activity_per_delegate): {:?}", e);
