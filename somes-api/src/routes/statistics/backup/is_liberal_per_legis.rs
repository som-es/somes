...
pub struct LegisIsLiberalFilter {
...
pub async fn is_liberal_per_legis(
    ...
    Json(filter): Json<Option<LegisIsLiberalFilter>>,
) -> Result<Json<Vec<LegisIsLiberal>>, StatisticsResponse> {
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, LegisIsLiberal>(&query);
