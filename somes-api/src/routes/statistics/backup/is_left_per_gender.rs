...
pub async fn is_left_per_gender(
    ...
    Json(filter): Json<Option<GenderIsLeftFilter>>,
) -> Result<Json<Vec<GenderIsLeft>>, StatisticsResponse> {
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, GenderIsLeft>(&query);
