...
pub struct PartyDivisonAccuracyFilter {
...
pub async fn division_accuracy_score_per_party(
    ...
    Json(filter): Json<Option<PartyDivisonAccuracyFilter>>,
) -> Result<Json<Vec<PartyDivisionAccuracy>>, StatisticsResponse> {
    let filter = filter.unwrap_or_default();

    let filter_arg = filter.legis_period.with_sql_column("pf.legislative_period");
    let filter_arg1 = Manual("m.is_nr").into_filter_arg();
...
ORDER BY 
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, PartyDivisionAccuracy>(&query);
