...
pub struct PartyIsLiberalFilter {
...
pub async fn is_liberal_per_party(
    ...
    Json(filter): Json<Option<PartyIsLiberalFilter>>,
) -> Result<Json<Vec<PartyIsLiberal>>, StatisticsResponse> {
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, PartyIsLiberal>(&query);
