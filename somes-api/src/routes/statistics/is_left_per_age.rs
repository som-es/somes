...
pub async fn is_left_per_age(
    ...
    Json(filter): Json<Option<AgeIsLeftFilter>>,
) -> Result<Json<Vec<AgeIsLeft>>, StatisticsResponse> {
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, AgeIsLeft>(&query);
