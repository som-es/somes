...
pub async fn total_speeches_per_party(
    ...
    Json(filter): Json<Option<PartyTotalSpeechesFilter>>,
) -> Result<Json<Vec<PartyTotalSpeeches>>, StatisticsResponse> {
...
ORDER BY 
    ...
    let mut filtered_query = sqlx::query_as::<Postgres, PartyTotalSpeeches>(&query);
