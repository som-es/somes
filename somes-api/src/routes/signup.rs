...
use crate::{
    db::establish_connection, email::send_verification_mail,
    routes::signup::action::validate_info_already_in_use_redis, RedisConnection, PostgresConnection, server::AppState, internal_error,
};
...
pub async fn signup(
    State(AppState { redis_client, postgres_pool }): State<AppState>,
    // RedisConnection(mut redis_con): RedisConnection,
    // PostgresConnection(mut conn): PostgresConnection,
    Json(signup_info): Json<SignUpInfo>,
) -> Result<Json<()>, SignUpErrorResponse> {
    let mut redis_con = redis_client.get_async_connection().await.map_err(|_| SignUpErrorResponse::RedisGetKeys)?;
